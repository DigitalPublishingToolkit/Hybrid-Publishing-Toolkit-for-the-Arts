...
##Reader software 
Various applications for PCs, tablets and smartphones support an extended subset of the EPUB standard, allowing for extra features such as customized fonts, layout styling, etc. In the following section we will examine the most important applications for electronic reading on mobile devices, desktop and laptop computers, and the Web. We make a distinction between four so-called ecologies, with their own devices, distribution platforms and software: Apple's iOS, Google's Android, Amazon's Kindle and Kobo books.   
