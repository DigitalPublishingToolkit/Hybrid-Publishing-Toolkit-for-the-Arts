...
#####Anchor 
An anchor is a specific location within a HTML web page that can be jumped to after having clicked a hyperlink. 
...
A graphic file that consists of two or more image frames shown as moving images in a timed sequences; most often used for endlessly looping mini films or graphic animations that only last a few seconds.

#####API (Application Programming Interface) 
A standardized way for two software **applications** to talk to each other and use each other's functions. APIs are used in software development; users normally do not see them.
...
#####Blog 
Short for web log; usually a chronological and often personal record of thoughts, web links, events, or actions posted on the Web. Lately, the difference between blogs and news and opinion web sites has become blurry. 

#####Browser 
The software with which people access and read web sites. A web browser loads pages from servers, translates their internal **HTML** code into a formatted text, displays graphics, plays music and video, and provides links to related pages. Web browsers work on the basis of the file format and network standards of the World Wide Web that are defined by the World Wide Web Consortium. In practice, however, browser manufacturers extend or deviate from those standards, which can lead to **compatibility** problems of a web design.
...
Cascading Style Sheets are a **markup** language for formatting electronic text with typographical attributes (such as bold, italic, typeface, point size, color, margin etc.). CSS is most commonly used for (HTML) web pages and by extension also for EPUB; it is typically stored in a separate file so that content and visual layout of a document remain separated from each other. Cascading Style Sheets make it easy to design a typographic template for a whole website or ebook. To change the typography, one only needs to edit the CSS file. By using different alternative CSS files for the same document, it also is easy to create different typographies for different purposes without having to re-author the document itself.
...
#####Unix 
Unix is a computer **operating system** originally developed by the telephone company AT&T in the early 1970s. It was the first widely used operating system that allowed multiple users to simultaneously login and work on the same computer. Many core technologies of the **Internet** derived from Unix development, and most Internet servers today run modern versions of Unix (mostly **Linux**). Next to Linux,
Mac OS X is fully Unix-compatible, **iOS** and **Android** are based on Unix technology.
